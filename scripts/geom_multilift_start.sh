...
# Launch multi-drone geom_multilift + sync goto with bag recording.
...
PARAMS_TEMPLATE="$WS_DIR/src/geom_multilift/config/geom_multilift_realflight_params.yaml"
if [[ ! -f "$PARAMS_TEMPLATE" ]]; then
  echo "geom_multilift params template not found: $PARAMS_TEMPLATE" >&2
  exit 1
fi

PARAMS_DIR="$WS_DIR/tmp"
mkdir -p "$PARAMS_DIR"
PARAMS_FILE="${PARAMS_DIR}/geom_multilift_realflight_params_${DRONE_ID}_$(date +%Y%m%d_%H%M%S).yaml"
...
start_tmux_with_recording() {
  ...
  # LEFT: geom_multilift realflight + swarm coordinator (launch includes coordinator)
  ...
  tmux send-keys -t "$LEFT_PANE" "ros2 launch geom_multilift geom_multilift_realflight_sync.launch.py params_file:=${PARAMS_FILE_ESCAPED}" C-m
