...
# Launch multi-drone L2C + sync goto with bag recording.
...
DRONE_ID="${DRONE_ID:-0}"
SESSION_NAME="l2c_session_${DRONE_ID}"
...
PARAMS_TEMPLATE="$WS_DIR/src/l2c_pkg/config/l2c_realflight_params.yaml"
if [[ ! -f "$PARAMS_TEMPLATE" ]]; then
  echo "l2c params template not found: $PARAMS_TEMPLATE" >&2
  exit 1
fi

PARAMS_DIR="$WS_DIR/tmp"
mkdir -p "$PARAMS_DIR"
PARAMS_FILE="${PARAMS_DIR}/l2c_realflight_params_${DRONE_ID}_$(date +%Y%m%d_%H%M%S).yaml"
sed "s#^[[:space:]]*data_root:.*#    data_root: \"${TRAJ_DIR}\"#" "$PARAMS_TEMPLATE" > "$PARAMS_FILE"
PARAMS_FILE_ESCAPED="$(printf '%q' "$PARAMS_FILE")"

echo "Using L2C data root: $TRAJ_DIR"
...
start_tmux_with_recording() {
  ...
  # Create session and split layout: LEFT (l2c), RIGHT-TOP (sync_goto), RIGHT-BOTTOM (rosbag)
  ...
  # LEFT: L2C realflight + swarm coordinator (launch includes coordinator)
  ...
  tmux send-keys -t "$LEFT_PANE" "ros2 launch l2c_pkg l2c_realflight_sync.launch.py params_file:=${PARAMS_FILE_ESCAPED}" C-m
