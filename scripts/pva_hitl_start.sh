...
# Launch multi-drone PVA (HITL) + sync goto with bag recording.
# Requires a trajectory directory argument that is forwarded to PVA params
# and the sync_goto state machine. Each drone runs one PVA node, one coordinator,
# and one state machine instance.

DRONE_ID="${DRONE_ID:-0}"
SESSION_NAME="pva_hitl_session_${DRONE_ID}"
...
PARAMS_TEMPLATE="$WS_DIR/src/pva_control/config/pva_hitl_params.yaml"
if [[ ! -f "$PARAMS_TEMPLATE" ]]; then
  echo "PVA HITL params template not found: $PARAMS_TEMPLATE" >&2
  exit 1
fi

PARAMS_DIR="$WS_DIR/tmp"
mkdir -p "$PARAMS_DIR"
PARAMS_FILE="${PARAMS_DIR}/pva_hitl_params_${DRONE_ID}_$(date +%Y%m%d_%H%M%S).yaml"
sed "s#^[[:space:]]*data_root:.*#    data_root: \"${TRAJ_DIR}\"#" "$PARAMS_TEMPLATE" > "$PARAMS_FILE"
PARAMS_FILE_ESCAPED="$(printf '%q' "$PARAMS_FILE")"

echo "Using PVA HITL data root: $TRAJ_DIR"
...
start_tmux_with_recording() {
  ...
  # Create session and split layout: LEFT (PVA HITL), RIGHT-TOP (sync_goto), RIGHT-BOTTOM (rosbag)
  ...
  # LEFT: PVA HITL + swarm coordinator (launch includes coordinator)
  ...
  tmux send-keys -t "$LEFT_PANE" "ros2 launch pva_control pva_hitl_sync.launch.py params_file:=${PARAMS_FILE_ESCAPED}" C-m
