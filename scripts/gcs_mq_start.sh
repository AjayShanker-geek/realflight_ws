...
Options:
  -i, --drone-ids IDS     Comma-separated drone IDs (default: ${DRONE_IDS})
...
tmux send-keys -t "$LEFT_PANE" \
  "ros2 run zmq_state_bridge zmq_state_bridge_node --ros-args \
...
tmux send-keys -t "$RIGHT_PANE" \
  "ros2 run zmq_state_bridge gcs_swarm_coordinator_node --ros-args \
  --params-file $WS_DIR/src/zmq_state_bridge/config/gcs_swarm_coordinator.yaml \
  -p drone_ids_csv:=${DRONE_IDS} \
  -p use_vicon_altitude:=${USE_VICON_ALTITUDE}" C-m
