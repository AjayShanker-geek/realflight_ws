...
RealflightFollowTrajNode::RealflightFollowTrajNode(int drone_id, int total_drones)
  : Node("realflight_follow_traj_node_" + std::to_string(drone_id))
  , drone_id_(drone_id)
  ...
  , odom_ready_(false)
{
  ...
  csv_path_ = this->declare_parameter(
    "csv_path",
    "data/realflight_traj_new/drone_" + std::to_string(drone_id_) + "_traj_smoothed_100hz.csv");
  ...
  RCLCPP_INFO(this->get_logger(),
              "=== Realflight Follow Trajectory Node for Drone %d ===", drone_id_);
  ...
  state_sub_ = this->create_subscription<std_msgs::msg::Int32>(
    ...
    rclcpp::QoS(10).reliability(RMW_QOS_POLICY_RELIABILITY_RELIABLE),
    std::bind(&RealflightFollowTrajNode::state_callback, this, std::placeholders::_1));
  ...
  odom_sub_ = this->create_subscription<px4_msgs::msg::VehicleOdometry>(
    ...
    std::bind(&RealflightFollowTrajNode::odom_callback, this, std::placeholders::_1));
  
  RCLCPP_INFO(this->get_logger(), "Subscribed to odometry: %sout/vehicle_odometry", 
              px4_namespace_.c_str());
  
  // Timer (wall time for real flight)
  timer_ = this->create_wall_timer(
      std::chrono::duration<double>(timer_period_),
      std::bind(&RealflightFollowTrajNode::timer_callback, this));
    
  RCLCPP_INFO(this->get_logger(), "Timer initialized at %.0f Hz", 1.0/timer_period_);
}

std::string RealflightFollowTrajNode::get_px4_namespace(int drone_id)
{
  if (drone_id == 0) {
    return "/fmu/";
  } else {
...
bool RealflightFollowTrajNode::load_trajectory_from_csv(const std::string& filepath)
{
...
TrajectoryPoint RealflightFollowTrajNode::interpolate_trajectory(double t)
{
...
bool RealflightFollowTrajNode::all_drones_in_traj_state()
{
...
void RealflightFollowTrajNode::state_callback(const std_msgs::msg::Int32::SharedPtr msg)
{
...
void RealflightFollowTrajNode::swarm_state_callback(
...
void RealflightFollowTrajNode::odom_callback(
...
void RealflightFollowTrajNode::timer_callback()
{
  ...
  if (current_state_ == FsmState::TRAJ && traj_started_ && !traj_completed_) {
    ...
    if (!traj_time_initialized_) {
      traj_start_time_ = steady_clock_.now();
    ...
    double elapsed = (steady_clock_.now() - traj_start_time_).seconds();
...
void RealflightFollowTrajNode::publish_trajectory_setpoint(
...
void RealflightFollowTrajNode::send_state_command(int state)
{
