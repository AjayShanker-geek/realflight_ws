#!/usr/bin/env python3
"""
Convert 3quad trajectory .npy files into CSV for C++ consumption.
Default scenario: rg_001_-001_3quad_l=1m_100Hz_smooth_useThis
...
def main():
    ...
    scenario = root / "raw_data" / "3quad_traj" / "Planning_plots_multiagent_meta_evaluation (rg_001_-001_3quad_l=1m_100Hz_smooth_useThis)"
    out_dir = root / "data" / "preprocessed" / "rg_001_-001_3quad_l1m_100hz"
    out_dir.mkdir(parents=True, exist_ok=True)

    dt = parse_dt(scenario / "setting.txt") or DEFAULT_DT

    xl = np.load(scenario / "xl_traj.npy", allow_pickle=True)  # (T,13)
    xq = np.load(scenario / "xq_traj.npy", allow_pickle=True)  # (N,T,8)
    uq = np.load(scenario / "uq_traj.npy", allow_pickle=True)  # (N,T-1,4)
    kb = np.load(scenario / "Kfb_traj.npy", allow_pickle=True)  # (T-1,6,13)
    ...
    cable_mu = resample_polynomial_series(xq[:, :, 6:7], time_raw, time_dense)[..., 0]

    time_u = np.arange(uq.shape[1]) * dt
    omega_dot_raw = uq[:, :, 0:3]
    time_u, omega_dot_raw = extend_time_series(time_u, omega_dot_raw, time_raw[-1], time_axis=1)
    omega_dot = resample_polynomial_series(omega_dot_raw, time_u, time_dense)
