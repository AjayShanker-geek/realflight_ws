#!/usr/bin/env python3
"""
Launch smooth-feedback PVA control nodes for SITL swarm.
Similar to pva_bounded_sitl_swarm.launch.py but uses the jerk-limited filter node.
"""
...
def make_nodes(context):
  ...
  nodes = [
    LogInfo(msg=f"Starting smooth PVA SITL swarm with {total} drones (sync_goto={launch_sync_goto})")
  ]
  ...
  for drone_id in drone_ids:
    ...
    nodes.append(
      Node(
        ...
        name=f"pva_smooth_sitl_{drone_id}",
        output="screen",
...
def generate_launch_description():
  ...
  default_params = PathJoinSubstitution([
    ...
    "config",
    "pva_smooth_feedback_params.yaml",
  ])
  return LaunchDescription([
    ...
    DeclareLaunchArgument("params_file", default_value=default_params,
                          description="YAML file with smooth PVA SITL parameters"),
