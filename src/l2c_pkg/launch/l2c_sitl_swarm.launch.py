#!/usr/bin/env python3
"""
Launch all SITL L2C nodes in one shot.
...
def make_nodes(context):
  ...
  nodes = [
    LogInfo(msg=f"Starting SITL L2C swarm with {total} drones (sync_goto={launch_sync_goto})")
  ]
  ...
  for drone_id in drone_ids:
    ...
    debug_log_path = ""
    if log_dir:
      log_path = os.path.join(log_dir, f"l2c_sitl_drone_{drone_id}.csv")
      debug_log_path = os.path.join(log_dir, f"l2c_sitl_debug_drone_{drone_id}.csv")
    params = [params_file]
    if log_path or debug_log_path:
      override = {}
      if log_path:
        override["log_path"] = log_path
      if debug_log_path:
        override["debug_log_path"] = debug_log_path
      params.append(override)
    nodes.append(
      Node(
        package="l2c_pkg",
        executable="l2c_node",
        name=f"l2c_sitl_{drone_id}",
        output="screen",
